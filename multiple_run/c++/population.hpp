...
private:
  ...
  // the number of individuals for each genotype
  std::vector<int> ind_nums;
  // fitness of each individual as a male (Wm)
  std::vector<double> fitness_male;
  // fitness of each individual as a female (Wf)
  std::vector<double> fitness_female;

  // optimum expression profiles
  ...
  // the last index of extant genotype in the vector pop
  int max_index;
  // whether to collect the same genotypes
  int mode;

  // whether intermediate phenotypes exist or not
  bool dimorphic;
