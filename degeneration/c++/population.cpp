...
Population::Population(const Parameters input_para,
  ...
  pop.emplace_back(para.pop_size, input_genotype, input_genotype, 
    input_interactions, input_interactions, 0, 0);
  max_index = 0;
  mode = 1;
...
void Population::development(const bool rand){
  ...
  for(int i = 0; i <= max_index; i++){
    ...
    if(old_sd_index == -1){
      ...
      int num = pop.at(i).ret_ind_num();
      int del = pop.at(i).ret_del_num();

      del_allele += num * del;

      if(del == 0 || rand == 0){
        ind_nums.push_back(num);
      }else if(del == 1){
        boost::random::binomial_distribution<> survive(num, std::max(0.0, 1.0 - para.s));
        ind_nums.push_back(survive(mt));
      }else{
        boost::random::binomial_distribution<> survive(num, std::max(0.0, 1.0 - 2.0 * para.s));
        ind_nums.push_back(survive(mt));
      }
    }

    if(ret.at(0) > para.criteria){
      male = 1;
    }
    if(ret.at(1) > para.criteria){
      female = 1;
    }
  ...
  if(male == 0 || female == 0){
    // considered dimorphic only when both sexes exist
    dimorphic = 0;
  }
}


// This function must be called after development(1)
void Population::selection(){
  ...
  bool allele1_male, allele2_male, allele1_female, allele2_female;
    ...
    while(success == 0){
      ...
      if(uni(mt) * max_fit < fitness){
        ...
        if(pair_order){
          ...
          allele1_male = pop.at(parent1).return_allele1();
          allele2_male = pop.at(parent1).return_allele2();
          ...
          allele1_female = pop.at(parent2).return_allele1();
          allele2_female = pop.at(parent2).return_allele2();

          fitness_male2.push_back(fitness_male.at(parent1));
          fitness_female2.push_back(fitness_female.at(parent2));

          count_mut_male += pop.at(parent1).ret_del_num();
          count_mut_female += pop.at(parent2).ret_del_num();
        }else{
          ...
          allele1_female = pop.at(parent1).return_allele1();
          allele2_female = pop.at(parent1).return_allele2();
          ...
          allele1_male = pop.at(parent2).return_allele1();
          allele2_male = pop.at(parent2).return_allele2();

          fitness_male2.push_back(fitness_male.at(parent2));
          fitness_female2.push_back(fitness_female.at(parent1));

          count_mut_male += pop.at(parent2).ret_del_num();
          count_mut_female += pop.at(parent1).ret_del_num();
        }
        ...
        for(int j = 0; j < para.num_g; j++){
          if(j != old_sd_index){
            ...
            if(p(mt)){
              geno1_male.at(j) = geno2_male.at(j);
              allele1_male = allele2_male;
            }
            if(p(mt)){
              geno1_female.at(j) = geno2_female.at(j);
              allele1_female = allele2_female;
            }
          }

          for(int k = 0; k < para.num_g; k++){
            if((j + 1) * para.num_g + k != old_sd_index){
    ...
    if(mode == 1 || gen % 100 == 0){
      ...
      for(int j = 0; j <= max_index; j++){
        if(next_gen.at(j).check_identical(geno1_male, geno1_female, 
          inte1_male, inte1_female, allele1_male, allele1_female)){
          success2 = j;
          break;
        }
      }
...
void Population::return_fitness(std::vector<double>& regi) const{
  ...
  for(int i = 0; i < para.pop_size; i++){
    fit_male += fitness_male2.at(i);
    fit_female += fitness_female2.at(i);
  }

  std::vector<double> ret = {fit / sum_ind, 
    fit_male / para.pop_size, fit_female / para.pop_size,
    count1 / sum_ind, 
    count2 / sum_ind, count3 / sum_ind, 
    1.0 * sum_ind, 
    count_mut_male / 2.0 / para.pop_size, count_mut_female / 2.0 / para.pop_size};
...
void Population::return_genotypes(std::vector<int>& num_dist,
  ...
  for(int i = 0; i <= max_index; i++){
    ...
    pop.at(i).return_interactions1(tmp_i1);
    pop.at(i).return_interactions2(tmp_i2);
    a1 = pop.at(i).return_allele1();
    a2 = pop.at(i).return_allele2();
...
void Population::return_mean_vars(std::vector<double>& means,
  ...
  std::vector<double> tmp_sum(para.num_g * (para.num_g + 1) + 1, 0);
  std::vector<double> tmp_sq_sum(para.num_g * (para.num_g + 1) + 1, 0);
  ...
  for(int i = 0; i <= max_index; i++){
    ...
    {
      int index = para.num_g * (para.num_g + 1);
      tmp_sum.at(index) += (a1 + a2) * num_size;
      tmp_sq_sum.at(index) += (a1 * a1 + a2 * a2) * num_size;
    }
  }
...
void Population::introduce_del(){
  ...
  std::ofstream ofs("log_pval_list.txt");

  std::vector<int> indices(log_p_val.size());
  for (int i = 0; i < static_cast<int>(indices.size()); i++) {
    indices.at(i) = i;
  }

  std::sort(indices.begin(), indices.end(), [&log_p_val](double a, double b) {
    return log_p_val.at(a) < log_p_val.at(b);
  });

  for(int i = 0; i < para.num_g * (para.num_g + 1); i++){
    ofs << indices.at(i) << "\t" << log_p_val.at(indices.at(i)) << std::endl;

    // loci showing strongest association
    if(i == 0){
      std::unordered_map<double, int> tmp_map;

      // check the frequency of homozygosity to distinguish X an Y (or Z and W) alleles
      for(int j = 0; j <= max_index; j++){
        pop.at(j).return_locus(indices.at(i), ret1, ret2, ret_ind_num);

        if(ret1 == ret2){
          if(tmp_map.count(ret1) == 0){
            tmp_map.emplace(ret1, ret_ind_num);
          }else{
            tmp_map.at(ret1) += ret_ind_num;
          }
        }
      }
      ...
      // the allele that is observed most frequently in homozygotes is regarded X or Z allele
      ...
      old_sd_index = indices.at(i);

      // introduce mutation into non-X or non-Z allele
      for(int j = 0; j <= max_index; j++){
        pop.at(j).add_del(indices.at(i), val);
      }
    }
  }
}
...
void Population::output_freq_origin(std::ofstream& ofs){
  std::vector<double> log_p_val;
  return_log_association(log_p_val);
  ...
  for(int i = 0; i < para.num_g * (para.num_g + 1); i++){
    for(const auto j: tmp.at(i)){
      ofs << gen - 1 << "\t" << i << "\t" << log_p_val.at(i) << "\t" << j.first << "\t" 
