...
void Genotype::add_mut_specified(const int index, const double effect, const bool parent){
  if(parent){
    if(index < num_g){
      genotype1.at(index) = effect;
    }else{
      int locus1 = (index - num_g) / num_g;
      int locus2 = (index - num_g) % num_g;

      interactions1.at(locus1).at(locus2) = effect;
    }
  }else{
    if(index < num_g){
      genotype2.at(index) = effect;
    }else{
      int locus1 = (index - num_g) / num_g;
      int locus2 = (index - num_g) % num_g;

      interactions2.at(locus1).at(locus2) = effect;
    }
  }
}

void Genotype::add_ac(const int locus_index, const double effect, int& sum){
  if(locus_index < num_g){
    if(genotype1.at(locus_index) == effect){
      sum += ind_num;
    }
    if(genotype2.at(locus_index) == effect){
      sum += ind_num;
    }
  }else{
    ...
    if(interactions1.at(locus1).at(locus2) == effect){
      sum += ind_num;
    }
    if(interactions2.at(locus1).at(locus2) == effect){
      sum += ind_num;
    }
  }
}