...
void prepare_files(const std::string seed_list, const std::string outcome, 
  const std::string freq, int& rep, int& success_test);
...
int main(int argc, char* argv[]){
  ...
  prepare_files("seed_list.txt", "outcome.txt", "freq.txt", ini_rep, success_test);
  ...
  for(int rep = ini_rep; rep <= max_rep; rep++){
    ...
    // run until only a single locus is polymorphic
    for(int i = 0; i <= max_time; i++){
      pop.one_generation();

      if(pop.return_max_index() <= 3){
        locus = pop.single_polymorphic_locus();
        
        if(locus >= 0){
          break;
        }else if(locus == -2){
          ofs2 << rep << "\t-3\t0\t0\tNo_polymorphism!" << std::endl;
          break;
        }
      }

      if(i == max_time){
        ofs2 << rep << "\t-2\t0\t0\tsingle_locus_div_has_not_been_reached" << std::endl;
      }
    }

    // if the polymorphic locus is not specified ancestral locus, disregard this replicate
    if(locus >= 0){
      if(locus != anc_locus){
        ofs2 << rep << "\t-1\t0\t0\tdifferent_locus" << std::endl;
        continue;
      }
    }else{
      continue;
    ...
    for(int rep2 = 1; rep2 <= rep_each_state; rep2++){
      ...
      for(int i = 0; i <= max_time; i++){
        ...
        if(i + 1 <= max_gen){
          pop.output_freq_site(der_locus, effect, freqs, i + 1);
        }
    ...
    ofs2 << rep << "\t" << 1 << "\t" << count.at(0) << "\t" << count.at(1) << "\t" << count.at(2) << std::endl;
    success_test++;
    
    if(success_test == max_test){
      break;
    }
  }
